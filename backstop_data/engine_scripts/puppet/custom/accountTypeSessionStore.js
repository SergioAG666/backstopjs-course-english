
module.exports = async page => {


    console.log('inicia accountTypeSessionStore');

    await page.evaluateOnNewDocument    (() => {

        sessionStorage.setItem('_xa_a', '{"name":"SERGIO","lastName":"ALONSO","middleName":"","secondLastName":"GARZON","customerIdentification":"79811281","phone":"3504676721","email":" ","nut":"AG798112812365378320","userIdentification":"79811281","habeasData":true,"checkSarlaft":true}');
        sessionStorage.setItem('_xa_a_c', '');
        sessionStorage.setItem('_a_cd', '');
        sessionStorage.setItem('_a_ot', '');

        sessionStorage.setItem('_a_ot_c', 'U2FsdGVkX1+tXO3W2FJfLn8bCy5Anm09AQIzIZm6pTA=');
        sessionStorage.setItem('_xj_a'  , '"eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiI0MzUzNDUzNDUiLCJyb2xlcyI6W3siaWQiOiIzIiwicm9sZSI6IkNVU1RPTUVSIn1dLCJleHAiOjE2MzU1NTQ2MjUsImlhdCI6MTYzNTU1MTAyNX0.GJfxcuof0iJ2ZsgD-n-I7LabL5IQTq9mWQcLgJt77ew"');
        sessionStorage.setItem('_xs_a_c', 'U2FsdGVkX18jeBgrghCMFtsT8FODa68W7/kNeyKEVhmlBBK6XD5fgtJfwUWzZAHSUHEuTe25tN1pXmn6mfvUBBe+GqzL5G2Fe5wJea32oIpSuQDH4UHcSm3rTSTwdZyUgSKj7f8FECU/yLSajWhmTQ==');
        sessionStorage.setItem('_xs_a'  , '{"identification":"0","name":"SERGIO ALONSO GARZON","office":"ADL","role":"CUSTOMER"}');

      });

}